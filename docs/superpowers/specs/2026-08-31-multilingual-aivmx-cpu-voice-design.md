...
合并第二批数据后，当前 `voice/haibara_jp/sliced` 有 120 条、总计约 412.6 秒的单声道 32 kHz 16-bit PCM WAV；28 条短于 2 秒。用户决定直接使用这批数据训练首个 `v0` 模型，不把继续补充素材作为开工条件。

这批数据足以验证预处理、训练、中文跨语言、ONNX 导出、AIVMX 导入和本地推理全链路。数据量较小可能导致音色覆盖、长句稳定性和普通话韵律上限较低，但是否可用仍由用户试听决定；如果 `v0` 通过主观验收，就不因时长不足强制重训。

### 6.2 真实日语数据

更高质量版本仍建议选择 1–3 小时干净、单人、录音条件相对一致的授权日语素材，但这只是失败后的改进路径，不是 `v0` 的前置条件。当前 120 条数据直接进入第一轮，以现有 2–6 秒完整句子为主体，台词逐字匹配实际发音。
