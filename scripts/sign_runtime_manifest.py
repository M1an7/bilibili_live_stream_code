...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create and optionally sign a BiliLiveTool GPU runtime manifest")
    parser.add_argument("--runtime-root", required=True)
    parser.add_argument("--runtime-id", default="gpt-sovits-cu126")
    parser.add_argument("--build-version", required=True)
    parser.add_argument("--gpt-sovits-commit", required=True)
    ...
    if not SHA256.fullmatch(args.gpt_sovits_commit):
