...
class RuntimeInstallJobManager:
    def __init__(self, installer: RuntimeInstaller, registry: RuntimeRegistry):
    ...
    def start(self, request: dict) -> str:
        if not isinstance(request, dict):
            raise RuntimeContractError("invalid_request", "GPU 运行时安装参数无效")
        source_type = request.get("source_type")
        source_path = str(request.get("path", "")).strip()
        if source_type not in ("zip", "directory") or not source_path:
            raise RuntimeContractError("invalid_request", "请选择 GPU 运行时 ZIP 或目录")
        with self._lock:
            if self._shutdown:
                raise RuntimeContractError("jobs_stopped", "GPU 运行时安装服务正在关闭")
            ...
            worker = threading.Thread(
                ...
                daemon=True,
                name=f"runtime-install-{job_id[:8]}",
            )
    ...
    def _run(self, job_id: str, source_type: str, source_path: str) -> None:
        try:
            self._update(job_id, status="running", message="正在检查 GPU 运行时")
            ...
            def progress(update: dict) -> None:
                self._update(
                    ...
                    message=update.get("message", "正在安装 GPU 运行时"),
                )
            ...
            self._update(
                job_id,
                status="failed",
                message="GPU 运行时安装失败，请查看应用日志",
                error={"code": "internal_error", "message": "GPU 运行时安装失败，请查看应用日志", "field": ""},
            )

    def get(self, job_id: str) -> dict:
        with self._lock:
            if job_id not in self._jobs:
                raise RuntimeContractError("job_not_found", "找不到 GPU 运行时安装任务")
